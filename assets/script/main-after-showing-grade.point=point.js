...
function updateCoursesTable() {
    ...
    courses.forEach((course) => {
        ...
        pointsCell.className = "text-xs py-2";

        const gradePoint = gradePoints[course.grade];
        const totalPoints = course.credit * gradePoint;

        pointsCell.innerHTML = `${course.credit} × ${gradePoint.toFixed(2)} = 
            <span class="text-green-500 font-semibold">${totalPoints.toFixed(2)}</span>`;
...
function updateRetakeCoursesTable() {
    ...
    retakeCourses.forEach((course) => {
        ...
        improvementCell.className = `text-xs py-2 ${improvement > 0 ? "text-green-500" : "text-amber-500"
            }`;
